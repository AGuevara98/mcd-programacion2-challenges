...
def preprocess_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out["clean_text"] = out["text"].astype(str).apply(preprocess_english_text)
    out["clean_text_length"] = out["clean_text"].str.len()
    out = out[out["clean_text"] != ""].reset_index(drop=True)

    print(f"English cleaned shape: {out.shape}")
    return out
...
def main(input_path: str, output_path: str) -> None:
    ...
    print(f"Saved cleaned English dataset to {output_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess English Glassdoor text.")
